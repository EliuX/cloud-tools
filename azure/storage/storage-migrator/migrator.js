...
export class StorageMigrator {
    ...
    async migrate() {
        ...
        try {
            ...
            // Synchronize Containers (if enabled)
            if (this.config.synchronizeContainers && this.blobMigrator) {
                console.log(chalk.yellow('\n📦 Synchronizing Blob Containers...'));
                const containerResults = await this.blobMigrator.synchronizeContainers();
                
                results.services.containers = containerResults;
                results.totalItems += containerResults.totalSourceContainers;
                results.migratedItems += containerResults.createdContainers + containerResults.updatedContainers;
                results.skippedItems += containerResults.skippedContainers;
                results.failedItems += containerResults.errors.length;
                results.errors.push(...containerResults.errors);
                
                console.log(chalk.green(`✅ Container synchronization completed: ${containerResults.createdContainers} created, ${containerResults.updatedContainers} updated, ${containerResults.deletedContainers} deleted`));
            }
    ...
    _printMigrationSummary(results) {
        ...
        if (results.services.containers) {
            const containers = results.services.containers;
            console.log(chalk.blue('\n📦 Container Storage:'));
            console.log(chalk.blue(`  Created: ${containers.createdContainers}`));
            console.log(chalk.blue(`  Updated: ${containers.updatedContainers}`));
            console.log(chalk.blue(`  Deleted: ${containers.deletedContainers}`));
            console.log(chalk.blue(`  Skipped: ${containers.skippedContainers}`));
        }
